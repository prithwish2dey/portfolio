...
const sortByDateDesc = (arr: any[]) =>
  [...arr].sort(
    (a, b) =>
      (parsePublishedDate(b)?.getTime() || 0) -
      (parsePublishedDate(a)?.getTime() || 0)
  );

/* ---------------- COLUMN ---------------- */

const PublicationColumn: React.FC<{
  title: string;
  publications: any[];
}> = ({ title, publications }) => (
  <div className="space-y-8">
    <motion.h3
      ...
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5 }}
      className="text-2xl sm:text-3xl font-bold text-cyan-400 text-center"
    >
      {title}
    </motion.h3>
    ...
    {publications.map((pub, index) => {
      ...
      const avatars =
        ...
        pub.authors?.slice(0, 5).map((a: string, i: number) => (
          <div
            key={i}
            className="w-8 h-8 rounded-full bg-cyan-800 text-white text-xs flex items-center justify-center"
          >
            {getInitials(a)}
          </div>
        ));
  ...
  >
    ...
    <div className="flex flex-col items-center mr-4">
      <div className="w-1 bg-cyan-400 rounded-full flex-1" />
      {/* Three colored dots */}
    ...
    {/* Publication card */}
    <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-4 sm:p-6 md:p-8 shadow-xl border border-cyan-500/10 flex-1">
      <h4 className="text-base sm:text-lg md:text-xl font-semibold bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500 break-words">
        {pub.title}
      </h4>
      ...
      <div className="mt-2 flex flex-col sm:flex-row sm:items-center gap-2 text-xs">
        ...
        <span className="text-gray-300">{pub.publisher}</span>
        {formatPublished(pub) && (
          <span className="sm:ml-auto bg-cyan-800/60 px-2 py-1 rounded-full text-cyan-200">
            {formatPublished(pub)}
          </span>
        )}
      </div>
        ...
        <div className="text-sm text-gray-300">
          ...
          {pub.authors.map((a: string, i: number) => (
            <span
              key={i}
              className={`ml-2 ${
                a === "Prithwish Dey" ? "text-cyan-400 font-semibold" : ""
              }`}
            >
              {a}
              {i < pub.authors.length - 1 ? "," : ""}
      ...
      <div className="mt-4">
        {hasLink ? (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 text-black text-sm font-medium hover:scale-105 transition"
          >
...
const Publications = ({
  ...
  const matches = (p: any) =>
    !query ||
    `${p.title} ${p.conference} ${p.journal} ${p.publisher} ${
      p.authors?.join(" ") || ""
    }`
      .toLowerCase()
      .includes(query.toLowerCase());

  const conf = conferencePublications.filter(matches);
  const jour = journalPublications.filter(matches);

  return (
    <section
      id="publications"
      className="relative w-full py-16 sm:py-20 md:py-24 px-4 sm:px-6 text-white bg-cover bg-center"
      style={{ backgroundImage: "url('/backgrounds/backpublication.webp')" }}
    >
      ...
      <div className="relative max-w-7xl mx-auto space-y-16">
        ...
        {!preview && (
          <input
            ...
            className="w-full max-w-xl mx-auto block px-4 py-2 rounded-full bg-black/30 border border-cyan-500/30"
          />
        )}
        ...
        <PublicationColumn
          ...
          publications={sortByDateDesc(
            tab === "all" ? [...conf, ...jour] : tab === "conference" ? conf : jour
          ).slice(0, preview ? limit : undefined)}
        />
        ...
        {preview && (
          <div className="text-center">
            <Link
              to="/publications"
              className="inline-flex items-center px-6 py-3 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 font-medium hover:scale-105 transition"
            >
              See all publications
            </Link>
          </div>
